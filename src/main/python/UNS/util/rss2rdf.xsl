...
<xsl:template match="item/description|r:item/r:description">
  <description><xsl:call-template name="removeTags" /></description>
...
<xsl:template match="item[link]|r:item[r:link]" mode="rdfitem">
  <item rdf:about="{link|r:link}">
    <xsl:apply-templates/>
        <!-- Nedjo addition: We don't want to us link as metadata element -->
        <link><xsl:value-of select="link|r:link" /></link>
  </item>
</xsl:template>
