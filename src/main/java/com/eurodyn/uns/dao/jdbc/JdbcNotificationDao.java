...
public class JdbcNotificationDao extends BaseJdbcDao implements INotificationDao {
    ...
    private static final String qNewNotifications = "SELECT N.ID, N.SUBJECT, N.CONTENT, N.HTML_CONTENT, " +
            ...
            "0 AS FAILED FROM (SELECT * FROM NOTIFICATION N WHERE NOT EXISTS (SELECT * FROM DELIVERY WHERE N.ID=NOTIFICATION_ID)) AS N, " +
            "SUBSCRIPTION S, SUBSCRIPTION_DT SDT WHERE N.EEA_USER_ID=S.EEA_USER_ID AND N.CHANNEL_ID=S.CHANNEL_ID AND S.ID=SDT.SUBSCRIPTION_ID " +
            "ORDER BY N.ID";
